...
class PPOAgent:
    def __init__(self, env_name, model_name=""):
        ...
        self.EPISODES = 10000 # quantidade de episodios
        self.episode = 0
        self.max_average = 0
        ...
        self.shuffle = True
        self.Training_batch = 256 # 256 instancias rodando ao msm tempo, como um multicore
        # cada instancia contribui para a atualização da política, mas cada uma subsequente a primeira tem menor importancia
    ...
    def discount_rewards(self, reward):
        gamma = 0.99 # hiperparametro, multiplica pela recompensa (demorar ou acelerar a conversão)
        running_add = 0 # nao sei de onde veio
    ...
    # imagino que era pra ser get_gates, apresenta os possíveis valores de "potencia" para os motores 
    ...
    def test(self, test_episodes = 100):
        self.load()
        for e in range(test_episodes + 1): # Corrected range to include the last episode
            # gymnasium update: env.reset() now returns a tuple (state, info)
            state, info = self.env.reset()
            state = np.reshape(state, [1, self.state_size[0]])
            done = False
            score = 0
            while not done:
                # In gymnasium, it's recommended to specify the render_mode at creation
                # e.g., self.env = gym.make(env_name, render_mode='human')
                # and then just call self.env.render() without arguments.
                self.env.render() 
...
if __name__ == "__main__":
    ...
    # rodar multicore
    #agent.run_multiprocesses(num_worker = 8)
